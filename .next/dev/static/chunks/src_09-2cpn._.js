...
'use client';
;
const STORAGE_KEY = 'racket_tournaments_v1';
...
function saveTournaments(tournaments) {
    ...
    try {
        ...
        // Dispatch custom event for same-tab updates and storage event for other tabs
...
const TournamentService = {
    ...
    updateMatch (tournamentId, matchId, payload) {
        ...
        if (payload.status === 'FINISHED' && payload.winnerId) {
            const winner = currentMatch.participant1?.id === payload.winnerId ? currentMatch.participant1 : currentMatch.participant2?.id === payload.winnerId ? currentMatch.participant2 : null;
        ...
        } else if (payload.status !== 'FINISHED' && currentMatch.nextMatchId && currentMatch.nextMatchSlot) {
            // If match was reverted back from FINISHED to LIVE/UPCOMING, clear advancement
            const nextIndex = matches.findIndex((m)=>m.id === currentMatch.nextMatchId);
            if (nextIndex !== -1) {
                const nextMatch = {
                    ...matches[nextIndex]
...
//# sourceMappingURL=src_09-2cpn._.js.map